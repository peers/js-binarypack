		...
		if(this.end < to){
			...
			switch(type){
				case 'arraybuffer':
					...
					if(this.index == this.end) this.refresh();
...
var _blober = (function(){
	...
	function _onData(e){
		...
		worker = undefined;
		var next = awaiting.shift();
		if(next) masterClock.after(0,doIt,doIt,next);
	}
	
	function doIt(ab,callback,that){
		if(worker){
			awaiting.push([ab,callback,that]);
			return;
		}
		
		worker = new Worker(file);
		worker.onmessage = _onData;
		worker._cb = callback;
		worker._that = that || w;
